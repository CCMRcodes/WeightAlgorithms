...
samp %>%
  ...
  ggplot(aes(x = value, fill = factor(weight_version))) +
  facet_wrap(vars(weight_version), nrow = 2) +
  geom_density() +
  geom_rug() +
  theme_void() +
  scale_fill_manual(values = c("#631647", "#FF8B0D"))
```
...
p3 <- rmplot(df = breland_df, title = "Breland et al. 2017")
...
p4 <- rmplot(df = maguen_df, title = "Maguen et al., 2013")
...
p5 <- rmplot(df = goodrich_df, title = "Goodrich et al., 2016")
...
p6 <- rmplot(df = chan_df, title = "Chan & Raffa, 2017")
...
p8 <- rmplot(df = buta_df, title = "Buta et al. 2018")
...
p9 <- rmplot(df = kazerooni_df, title = "Kazerooni & Lim, 2016")
...
p11 <- rmplot(rosenberger_df, title = "Rosenberger et al. 2011")
```

Comparison of Algorithms depicting algorithm-removed weights (red) juxtaposed with raw weight data (black):

```{r, cache = TRUE, eval = TRUE, echo = FALSE, fig.width = 14, fig.height = 16}
library(ggpubr)
figure <- ggpubr::ggarrange(p0, p1,  p2,
                            p3, p4,  p5,
                            p6, p7,  p8,
                            p9, p10, p11,
                            nrow = 4, ncol = 3)
